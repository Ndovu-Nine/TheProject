...
namespace TemboContext.Dapper
{
	...
	public static class SqlMapper
	{
		...
		private static void CollectCacheGarbage()
		{
			try
			{
			    foreach (var pair in _queryCache.Where(pair => pair.Value.GetHitCount() <= COLLECT_HIT_COUNT_MIN))
			    {
			        CacheInfo cache;
		...
		/*hit the server*/
		private const int COLLECT_PER_ITEMS = 3000, COLLECT_HIT_COUNT_MIN = 0;
		...
		public class Identity : IEquatable<Identity>
		{
			...
			{
				return new Identity(sql, commandType, connectionString, typeField, type, null, -1);
			}

			internal Identity(string sql, CommandType? commandType, IDbConnection connection, Type typeField, Type parametersType, Type[] otherTypes)
				: this(sql, commandType, connection.ConnectionString, typeField, parametersType, otherTypes, 0)
			{ }
			private Identity(string sql, CommandType? commandType, string connectionString, Type typeField, Type parametersType, Type[] otherTypes, int gridIndexField)
			{
				this.sql = sql;
				...
				this.gridIndexField = gridIndexField;
				unchecked
				{
					hashCode = 17; // we *know* we are using this in a dictionary, so pre-compute this
					hashCode = hashCode * 23 + commandType.GetHashCode();
					hashCode = hashCode * 23 + gridIndexField.GetHashCode();
					hashCode = hashCode * 23 + (sql?.GetHashCode() ?? 0);
					hashCode = hashCode * 23 + (typeField == null ? 0 : typeField.GetHashCode());
					if (otherTypes != null)
					{
			...
			/// 
			/// </summary>
			public readonly int hashCode, gridIndexField;
			...
			public bool Equals(Identity other)
			{
				return
					other != null &&
					gridIndexField == other.gridIndexField &&
...
this IDbConnection cnn, string sql, dynamic param = null, IDbTransaction transaction = null, int? commandTimeout = null, CommandType? commandType = null
		...
		{
			Identity identity = new Identity(sql, commandType, cnn, typeof(GridReader), ((object) param)?.GetType(), null);
		...
		{
			var identity = new Identity(sql, commandType, cnn, typeof(T), param?.GetType(), null);
...
this IDbConnection cnn, string sql, Func<TFirst, TSecond, TThird, TFourth, TReturn> map, dynamic param = null, IDbTransaction transaction = null, bool buffered = true, string splitOn = "Id", int? commandTimeout = null, CommandType? commandType = null
		...
		{
			identity = identity ?? new Identity(sql, commandType, cnn, typeof(TFirst), param?.GetType(), new[] { typeof(TFirst), typeof(TSecond), typeof(TThird), typeof(TFourth), typeof(TFifth) });
...
Type type, IDataReader reader, int startBound = 0, int length = -1, bool returnNullIfFirstMissing = false
		...
		public static void ThrowDataException(Exception ex, int index, IDataReader reader)
		{
			...
			throw new DataException($"Error parsing column {index} ({name}={value})", ex);
		}
		...
		public class GridReader : IDisposable
		{
			...
			private IEnumerable<TReturn> MultiReadInternal<TFirst, TSecond, TThird, TFourth, TFifth, TReturn>(object func, string splitOn)
			{
				...
				var forGrid = identity.ForGrid(typeof(TReturn), new[] {
                    typeof(TFirst),
                    typeof(TSecond),
                    typeof(TThird),
                    typeof(TFourth),
				...
				try
				{
					foreach (var r in MultiMapImpl<TFirst, TSecond, TThird, TFourth, TFifth, TReturn>(null, null, func, null, null, splitOn, null, null, reader, forGrid))
					{
						yield return r;
					}
				}
